        ...
        public static void AddGlobalHandlers()
        {
            AppDomain.CurrentDomain.UnhandledException += (sender, args) =>
            {
                try
                {
                    ...
                    var filePath = Path.Combine(LogsPath,
                        string.Format("UnhandledException_{0}.json", DateTime.Now.ToShortDateString().Replace("/", "-")));
            ...
            Application.ThreadException += (sender, args) =>
            {
                try
                {
                    ...
                    var filePath = Path.Combine(LogsPath,
                        string.Format("ThreadException_{0}.json", DateTime.Now.ToShortDateString().Replace("/", "-")));
